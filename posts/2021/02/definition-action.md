...
로 사상 $\Phi$를 정의하면, $\Phi$는 준동형이라는 것을 쉽게 확인할 수 있다. (연습문제)
...
$\Phi: G\to\text{Bij}(X)$에 의하여 정의되는 $G$의 $X$에 대한 작용을 **충실한 작용(faithful
action)** 이라고 한다.
...
**예 4**(치환표현). 군 $G$가 유한집합인
$X = \left\{ x_1,\ldots, x_n\right\}$에 작용한다고 하면, $i=1,\ldots, n$에 대하여
$x_{\rho(g)(i)} = g\cdot x_i$ 를 만족하는 $\rho(g)\in \mathfrak S_n$가 유도된다는 것을 명제 1로부터 알
