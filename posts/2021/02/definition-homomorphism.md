**정의 1**(준동형, 동형).  군 $G_1, G_2$에 대하여, 사상 $\phi: G_1 \to G_2$가
...
$\phi$가 동형이라면, $\phi(\phi^{-1}(ab)) = ab =
\phi(\phi^{-1}(a))\phi(\phi^{-1}(b)) = \phi(\phi^{-1}(a)\phi^{-1}(b))$ 이
성립하고, $\phi$는 단사이므로, $\phi^{-1}$ 역시 준동형을 이룬다는 것을 알 수
있다.
...
**명제 2**.  $\phi,\psi\colon G_1\to G_2$가 준동형이라고 하자. 또한, $S\subset
G_1$에 대하여 $ \langle S \rangle = G_1$일 때, 임의의 $x\in S$에 대하여
...
1. $\text{Ker}(\phi) = \left\\{ x\in G\_1 \mid \phi(x) = 1\_{G\_2} \right\\}$을
