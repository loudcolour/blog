...
**정의 1**(가환자군). $G$를 [군](/2021/02/definition-group/)이라고 하자.
...
군 $G$가 주어졌을 때, 가환자군을 반복하여 얻어서 군의 열을 만들 수 있다.
...
- **가환자** 와 **가환자군** 을 정의했다.
- 군 $G$의 가환자군 $D(G)$는 정규부분군이고, 임의의 $N\vartriangleleft G$에
  대하여, $G/N$이 abelian일 필요충분조건이 $D(G) \subset N$임을 보였다.
- 가환자군을 이용하여 **가해군** 을 정의했다. 또한 이 정의가 (정리 4를 통하여)
  정규부분군의 사슬을 이용한 가해군의 정의와 동등함을 보였다.
- 가해군이 아닌 군의 예시로서 **단순군** 을 정의했다.
