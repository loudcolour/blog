...
**증명**.  먼저 $H\in X$에 대하여, $\pi^{-1}(H)\in Y$임을 보이자.  즉, $G/N$의
부분군인 $H$에 대해, $\pi^{-1}(H)$이 $G$의 부분군, 동시에 $N$을 포함하는 것을
보인다.

- $N = \left\{ 1_{G/N} \right\} \subset H$.
- $\pi^{-1}(H)$는 아래의 조건을 만족하므로 $G$의 부분군.
    - $1_G \in N \subset \pi^{-1}(H)$.
    - $x,y \in \pi^{-1}(H)$라고 하면, $\pi(x), \pi(y)\in H$, $\pi(xy)=\pi(x)\pi(y)\in H$이므로,
      $xy\in \pi^{-1}(H)$.
    - $x\in \pi^{-1}(H)$라고 하면, $\pi(x)\in H$, $\pi(x^{-1}) = \pi(x)^{-1}\in H$이므로,
      $x^{-1}\in \pi^{-1}(H)$.

따라서 $\pi^{-1}(H) \in Y$, $\phi$는 사상.

다음으로, $\psi$가 사상임을 보이자.
$\phi$와 마찬가지로, 군 $G$의 $N$을 포함하는 부분군 $K\subset G$에 대하여
$\pi(K)\in X$, 즉 $\pi(K)$가 $G/N$의 부분군임을 보이면 된다.

$N$은 $G$의 정규부분군이므로, $K \vartriangleright N$,
$\pi(K) = \left\{ gN \,|\, g\in K \right\} = K/N\subset G/N$이다.
$\pi|_K: K \to G/N$은 준동형이므로, $\pi(K) = \text{Im}(\pi|_K)$는 $G/N$의
부분군, 따라서 $\pi(K)\in X$이다.
