...
$f$에 의하여 $a = f(\Delta(W,a))$가 성립한다면, $W$가 $f$-열인 정렬집합이라고
하자. 또한, $f$-열인 정렬집합 전체의 집합을 $\mathfrak{F}$라고 하자.

우선, 임의의 $W_1, W_2\in \mathfrak{F}$에 대하여, $W_1 = W_2$ 혹은 $W_1$과
$W_2$중 하나가 다른 하나의 절편과 일치하는 것을 보이자. $W_1$과 $W_2$는 모두
정렬집합이고, 정렬집합의 비교정리에 의하여 $W_1\cong W_2$ 혹은 $W_1$와 $W_2$ 중
하나가 다른 하나의 절편과 순서동형을 이룬다. 그 중, $a\in W_2$에 대하여
$W_1\cong W_2 \langle a \rangle$인 경우를 고려하자. 여기서 순서동형사상
$\phi: W_1\to W_2 \langle a \rangle$에 대하여 $\phi(x) = x$임을 보인다면,
$W_1$이 $W_2$의 절편과 일치함을 보일 수 있다.
...
$m\in W_1'$에 모순이므로, $W_1'=\emptyset$이다. 따라서 $W_1$는 $W_2$의 절편과
일치한다. $W_2$가 $W_1$의 절편과 순서동형일 때, $W_1\cong W_2$일 때에도 같은
방법으로 위의 주장이 성립함을 보일 수 있다.

위의 결과와, 정렬집합의 구성정리에 의하여,
$W_\infty\coloneqq \bigcup_{W\in \mathfrak{F}}W$도 정렬집합이다. 그리고
$W_\infty\in \mathfrak{F}$가 성립하고[^1], $X$는 귀납적인 순서집합이므로, 그 정렬부분집합(즉, 전순서집합)
$W_\infty$는 상계인 $w\in X$를 갖는다. $w\in X$는 $X$의 극대원소이므로[^2],
Zorn의 보조정리가 성립한다. $\square$
...
$f(\Delta(W_\infty, a)) = f(\Delta(W, a)) = a$이 성립, $W_\infty \in \mathfrak{F}$
이다.)
