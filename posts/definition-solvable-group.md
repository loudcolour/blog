...
군 $G$와 그 부분군 $H, N$이 주어지고, 특히 $N$은 $G$의 [정규부분군](/definition-normal-subgroup/)이라고 하자.
이 때,
...
**주**. $G'$이 abelian, $f: G\to G'$이 [준동형](/definition-group-homomorphism/)이라고 하자.
[준동형정리](/homomorphism-theorem/)에 의하여
$G/\text{Ker}(f) \cong \text{Im}(f) \subset G'$은 abelian, 따라서,
$D(G)\subset \text{Ker}(f)$임을 이 명제를 통하여 알 수 있으므로, 자연스러운 준동형
$\pi: G\to G/D(G)$가 주어졌을 때, [준동형의 분해](/homeomorphism-theorem-3/#준동형의-분해)를 통해 $g\circ \pi = f$를
